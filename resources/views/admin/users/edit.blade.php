    ...
    <div class="container pt-3">
        @include('messages')
            <div class="card">
                ...
                <div class="card-body">
                    <form id="create_user" name="create_user" action="{{ route('users.update', $user->id) }}" method="post">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="last_name" id="last_name" placeholder="Enter First Name" value="{{ old('last_name', $user->last_name) }}">
                                @error('last_name')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="selectTenant" name="tenant_id">
                                        ...
                                        @foreach($tenants as $tenant)
                                            <option value="{{ $tenant->id }}" {{ $user->user_id == $tenant->id ? 'selected' : '' }}>{{ $tenant->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('tenant_id')
                                    <span class="text-danger">{{ $message }}</span>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control" name="google2fa_enable">
                                    <option value="1" {{ $user->google2fa_enable ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ !$user->google2fa_enable ? 'selected' : '' }}>No</option>
                                </select>
